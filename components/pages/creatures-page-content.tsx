...
import { CreatureCard } from "@/components/cards/creature-card/creature-card";
import { FilterBar } from "@/components/ui/filter-bar/filter-bar";
import { useTranslations } from "@/hooks/use-translations";
import { useFilters } from "@/hooks/use-filters";
import { CREATURE_FILTERS } from "@/constants/filters.constants";
import type { Creature } from "@/types/creature.types";
import { Dog } from "lucide-react";

interface CreaturesPageContentProps {
  creatures: Creature[];
}

export function CreaturesPageContent({ creatures }: CreaturesPageContentProps) {
  ...
  const {
    ...
    hasActiveFilters,
    enrichedConfigs,
  } = useFilters(creatures, CREATURE_FILTERS);
          ...
          <div className="flex justify-center mt-8">
            <div className="relative w-48 h-px bg-gradient-to-r from-transparent via-[var(--gold)] to-transparent">
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[var(--background)] px-4">
                <Dog className="w-6 h-6 text-[var(--gold)]" strokeWidth={2} />
              </div>
            </div>
          </div>
        </div>
        ...
        {filteredData.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-8">
            {filteredData.map((creature) => (
              <CreatureCard key={creature.identifier} creature={creature} />
            ))}
          </div>
        ) : (
          <div className="text-center py-32">
            <div className="mb-6 flex justify-center">
              <Dog className="w-16 h-16 text-[var(--gold)]" strokeWidth={1.5} />
            </div>
            <p className="text-2xl font-bold text-[var(--gold)] mb-4">
              {t.pages.creatures.empty.title}
            </p>
            <p className="text-lg text-[var(--text-secondary)]">
              {t.pages.creatures.empty.description}
            </p>
          </div>
        )}
      </div>

...
/**
 * Componente cliente para mostrar los detalles de una criatura
 * con soporte multiidioma
 */
export function CreatureDetailContent({
  creature,
}: CreatureDetailContentProps) {
  const { language } = useLanguage();
  const t = useTranslations();

  const currentName = language === "es" ? creature.name.es : creature.name.en;
  const currentDescription =
    language === "es" ? creature.description.es : creature.description.en;
  const currentRace = language === "es" ? creature.race.es : creature.race.en;
          ...
          <div>
            <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
              <div className="p-1.5 sm:p-2 bg-[var(--gold)]/10 border border-[var(--gold)]/30 rounded-lg sm:rounded-xl">
                <Dog className="w-6 h-6 sm:w-8 sm:h-8 text-[var(--gold)]" />
              </div>
            ...
            {/* Race Badge */}
            <div className="inline-flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 bg-[var(--surface)] border border-[var(--border)] rounded-lg sm:rounded-xl">
